...
try:
    ...
    print("In order to use qa, please install the 'qa_generation' extra")
    raise e
    ...
    while True:
        try:
            ...
            if n > _MAX_RETRIES:
                raise
            secs = 2**n
            message = f"Retrying after {secs}s. API call failed due to {_re.__class__.__name__}: {_re}"
...
class QADataGenerator:
    ...
    def __init__(self, model_config: Dict):
        ...
        self._chat_completion_params = dict(
            ...
            api_type=(
                model_config["api_type"] if "api_type" in model_config else os.getenv("OPENAI_API_TYPE", "azure")
            ),
            api_version=(
    ...
    @distributed_trace
    @monitor_operation(activity_name="DataGenerator.Export", activity_type=ActivityType.INTERNALCALL)
    def export_to_file(
        self,
        output_path: str,
        qa_type: QAType,
    ...
    @distributed_trace
    @monitor_operation(activity_name="DataGenerator.Generate", activity_type=ActivityType.INTERNALCALL)
    ...
    @distributed_trace
    @monitor_operation(activity_name="DataGenerator.GenerateAsync", activity_type=ActivityType.INTERNALCALL)
